...
# Helper class for persistent file storage
...
def delete_page():
    ...
    if "pages" in st.session_state and 0 <= st.session_state.current_page < len(st.session_state.pages):
        ...
        if st.session_state.pages:
            ...
            st.session_state.editor = "" # Empty state
...
def main():
    ...
    # Start Over Button
    st.sidebar.button("🔄 Start Over", on_click=reset_app)
    ...
    if uploaded_file is not None:
        # Check if it's a new upload or different file
        # Simple check: name and size. Or just assume uploader widget is truth.
        # If widget has value, we update our stored bytes.
        
        # If stored name or size is different, update stored bytes
        is_new_file = False
    ...
    # Prepare active_file from session state if available
    if st.session_state.uploaded_file_bytes is not None:
        active_file = NamedBytesIO(
            st.session_state.uploaded_file_bytes,
            st.session_state.uploaded_file_name,
            st.session_state.uploaded_file_size
        )

    # Determine ID for processing logic
    ...
    if active_file is not None:
        ...
        if st.session_state.pages:
            ...
            with col4:
                st.button("🗑️ Delete Page", on_click=delete_page, type="primary")
                ...
                if not full_text.strip():
                    ...
                    with st.spinner("Generating audio..."):
                        try:
                            ...
                            # Update Audio Player with correct MIME
                            st.audio(audio_bytes, format="audio/mpeg")

                            # Update Download Button with correct MIME
                            st.download_button(
                                label="Download MP3",
                                data=audio_bytes,
            ...
            # Empty state message if pages were deleted
            if active_file is not None:
                st.info("Document is empty. All pages were deleted.")

    else:
        # Reset state if file is removed
        # If active_file is None, it means no uploaded_file AND no stored bytes.
